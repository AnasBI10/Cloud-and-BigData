...
**Commit-Verteilung (Git-History gegengeprueft):** Da manche Personen unter mehreren E-Mail-Adressen committet haben (z. B. Uni- und Arbeits-Account), wurde die reine `git shortlog`-Zaehlung dahingehend bereinigt (`git log --format='%ae'` je Alias zusammengefuehrt) und ergibt folgendes Bild ueber alle 90 Commits: Anas Boudhaim 45, victortschauder 30, Buenyamin Tasci 7, Gettogoofy 5, Jessica Ruppel 2, David 1. Die Verteilung ist damit ungleich, aber plausibel: Anas Boudhaim und victortschauder haben durchgaengig am Projekt mitgearbeitet (Deployment, Processing), waehrend Buenyamin Tasci, Gettogoofy, Jessica Ruppel und David sich auf klar abgegrenzte Teilbereiche (Serving/UI, Ingestion, punktuelle Beitraege, Architekturdiagramm) konzentriert haben. Die History enthaelt einen einzelnen, bewusst so benannten Zwischenstand (`WIP SCRUM-83: ... (unfertig)`), der spaeter fertiggestellt und in Folge-Commits abgeschlossen wurde -- ehrlich stehengelassen statt nachtraeglich per History-Rewrite verschleiert.
